...
name <- c('jhon', 'peter', 'patrick', 'julie', 'bob')
name
class(name)
age <- c(28,30,15,13,46)
age
class(age)
children <- c(TRUE, FALSE, FALSE, TRUE, TRUE)
children
class(children)
...
height <- c(161,177,163,162,175)
df$height <- height
df

width <- c(75,85,48,63,25)
df$width <- width
#cbind(df, width)
operation <- (height * width)
cbind(df, operation  )

tom <- data.frame(name="Tom", age=37, children=FALSE, height=145, width=96)
df
rbind(df, tom)
...
df[,2] # Extract the second colunmw
